...
| 18 | `18-android-manifest-cleartext-flag.png` | Documents the exact, deliberate, temporary security trade-off made for device testing | `cat android/app/src/main/AndroidManifest.xml` | `android:usesCleartextTraffic="true"` visible (captured before removal; the manifest has no TEMP comment, the temporary status is documented in ADR-003) | SUPPORTING (pairs with ADR-003) |
